define([
    ...
    imagesCollection.loadImages = function() {
        webix.ajax().get(app.config.defaultAPIPath + "/facets/images")
                ...
                if(obj.facets.hasOwnProperty((data[i].key))) {
                    if (data[i].status == "equals") {
                        if (data[i].value instanceof Array) {
                            ...
                            if (data[i].value == obj.facets[data[i].key]) {
                                show[data[i].key] = true;
                            }
                        }
                    } else if (data[i].status == "less") {
                        if (data[i].value < obj.facets[data[i].key]) {
                            show[data[i].key] = true;
                        }

                    } else if (data[i].status == "between") {
                        if (data[i].max >= obj.facets[data[i].key] && data[i].min <= obj.facets[data[i].key]) {
                            show[data[i].key] = true;
                        }
                    }
                }
